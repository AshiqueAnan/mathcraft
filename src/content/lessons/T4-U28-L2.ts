...
export const T4U28L2: Lesson = {
  ...
  intuitionBlocks: [{ widget: "number-line", props: { mode: "dots", dots: [24, 25, 26, 27, 95] }, narrative: "Place salaries as dots on a line. One giant outlier far right: the mean jumps way past where most dots sit, while the median stays near the crowd. Slide the outlier and watch the mean chase it — that's how a single number can misrepresent a whole dataset." }],
  ...
  quiz: {
    pool: [
      // @q01
      { id: "U28L2-mcq-1", type: "mcq", category: "procedural", prompt: "Salaries: 40k, 42k, 45k, 50k, 3M. The MEAN ≈ …", options: [ { id: "a", text: "$635k" }, { id: "b", text: "$45k" }, { id: "c", text: "$50k" }, { id: "d", text: "$3M" } ], correctOptionId: "a", diagnoses: { b: "45k is the MEDIAN.", c: "50k is the max of the non-outliers.", d: "3M is one salary, not the average." }, explanation: "Sum ≈ 3.177M ÷ 5 ≈ $635k — dragged up by the outlier.", hints: ["Sum ÷ 5.", "≈ $635k.", "Sum ≈ 3.177M ÷ 5 ≈ $635k — dragged up by the outlier."] },
      // @q02
      { id: "U28L2-mcq-2", type: "mcq", category: "conceptual", prompt: "For 'typical worker salary' the honest choice is…", options: [ { id: "a", text: "mean" }, { id: "b", text: "median" }, { id: "c", text: "mode" }, { id: "d", text: "max" } ], correctOptionId: "b", diagnoses: { a: "The mean is inflated by executives.", c: "Mode shows the most common single value — useful but blunt.", d: "Max describes the richest, not the typical." }, explanation: "The median sits at the middle worker — immune to the executive tail.", hints: ["Middle worker.", "Median.", "The median sits at the middle worker — immune to the executive tail."] },
      // @q03
      { id: "U28L2-mcq-3", type: "mcq", category: "word", prompt: "A news headline: 'House prices average $900k!' — but most homes sell ~$350k. The reporter likely used the…", options: [ { id: "a", text: "mode, correctly" }, { id: "b", text: "median, correctly" }, { id: "c", text: "mean, skewed by a few mansions" }, { id: "d", text: "range, correctly" } ], correctOptionId: "c", diagnoses: { b: "The median would be near $350k.", a: "The mode is the most common price, not the average.", d: "The range is min-to-max, not an average." }, explanation: "A few extreme sales inflate the mean while the bulk stays low.", hints: ["Mansions skew.", "Mean.", "A few extreme sales inflate the mean while the bulk stays low."] },
      // @q04
      { id: "U28L2-mcq-4", type: "mcq", category: "procedural", prompt: "Data: 100, 105, 110, 115. Remove the outlier 1000 from 100, 105, 110, 115, 1000. The MEDIAN…", options: [ { id: "a", text: "becomes 0" }, { id: "b", text: "doubles" }, { id: "c", text: "becomes 1000" }, { id: "d", text: "barely changes" } ], correctOptionId: "d", diagnoses: { b: "Median doesn't scale with values.", c: "1000 is the outlier, not the middle.", a: "No value becomes zero." }, explanation: "With 1000: median = 110; without: median = 107.5 — a small shift.", hints: ["Middle values only.", "110 vs 107.5.", "Barely changes."] },
      // @q05
      { id: "U28L2-mcq-5", type: "mcq", category: "conceptual", prompt: "When data is right-skewed (a long tail of big values), the mean is usually…", options: [ { id: "a", text: "greater than the median" }, { id: "b", text: "less than the median" }, { id: "c", text: "equal to the mode" }, { id: "d", text: "zero" } ], correctOptionId: "a", diagnoses: { b: "The tail pulls the mean UP, not down.", c: "Mode is about frequency, not location.", d: "Means can't vanish." }, explanation: "The tail drags the mean right of the median.", hints: ["Tail pulls.", "Mean > median.", "The tail drags the mean right of the median."] },
      // @q06
      { id: "U28L2-mcq-6", type: "mcq", category: "word", prompt: "A store averages 100 customers/day except Sundays (all staff + families = 600). Best 'busy typical day' summary?", options: [ { id: "a", text: "Mean ≈ 171 customers" }, { id: "b", text: "Median ≈ 100 from the six ordinary days" }, { id: "c", text: "Mode = 600" }, { id: "d", text: "Range = 100" } ], correctOptionId: "b", diagnoses: { a: "The Sunday outlier drags the mean up to 171.", c: "600 happens only once — it's not common.", d: "Range is min-to-max spread, not typical." }, explanation: "Six days near 100 with one 600-day: the median stays near the ordinary.", hints: ["Sunday skews.", "Use the middle.", "Six days near 100 with one 600-day: the median stays near the ordinary."] },
      // @q07
      { id: "U28L2-num-1", type: "numeric-input", category: "procedural", prompt: "Data: 2, 3, 3, 4, 88. Median = …", answer: 3, tolerance: 0, explanation: "Sorted: 2, 3, 3, 4, 88 → middle is 3.", hints: ["Sorted middle.", "3.", "Sorted: 2, 3, 3, 4, 88 → middle is 3."] },
      // @q08
      { id: "U28L2-num-2", type: "numeric-input", category: "procedural", prompt: "Same data: 2, 3, 3, 4, 88. Mean = …", answer: 20, tolerance: 0, explanation: "Sum 100 ÷ 5 = 20 — yanked up by 88.", hints: ["100 ÷ 5.", "20.", "Sum 100 ÷ 5 = 20 — yanked up by 88."] },
      // @q09
      { id: "U28L2-num-3", type: "numeric-input", category: "conceptual", prompt: "Add one huge value to 1, 2, 3, 4. The median of the 5 sorted values stays 3 because the 3rd position never moves. The added value can be as small as what number without becoming the median?", answer: 4, tolerance: 0, explanation: "With 5 sorted values the median is the 3rd. The added value ≥ 4 becomes the 5th, so the 3rd stays 3 — the outlier's SIZE doesn't matter, only that it doesn't displace the middle.", hints: ["5th position.", "Median is the 3rd.", "With 5 sorted values the median is the 3rd."] },
      // @q10
      { id: "U28L2-frac-1", type: "fraction-input", category: "conceptual", prompt: "In 1, 2, 2, 100 the mean is 26.25 but the median is 2. What fraction of the data lies at or below the median? (2 of 4 values)", numerator: 1, denominator: 2, acceptEquivalent: true, explanation: "Values 1 and 2 (two of four) sit at/below the median 2 → 1/2.", hints: ["2 of 4.", "1/2.", "Values 1 and 2 (two of four) sit at/below the median 2 → 1/2."] },
      // @q11
      { id: "U28L2-tf-1", type: "true-false-justify", category: "conceptual", prompt: "The mean can be greater than every value except the outlier in a right-skewed set.", isTrue: true, explanation: "A huge outlier drags the mean above most points — that's the lie.", hints: ["Outlier pulls.", "Mean above crowd.", "A huge outlier drags the mean above most points — that's the lie."] },
      // @q12
