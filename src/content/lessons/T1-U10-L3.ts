...
export const T1U10L3: Lesson = {
  ...
  quiz: {
    pool: [
      // @q01
      { id: "U10L3-mcq-1", type: "mcq", category: "procedural", prompt: "20 students: 12 like tea, 8 like coffee, 5 both. How many like tea ONLY?", options: [ { id: "a", text: "7" }, { id: "b", text: "12" }, { id: "c", text: "5" }, { id: "d", text: "17" } ], correctOptionId: "a", diagnoses: { b: "12 includes the 5 who also like coffee — subtract the overlap.", c: "5 is the 'both' count, not tea-only.", d: "17 is the union, not the tea-only slice." }, explanation: "Tea-only = 12 − 5 = 7.", hints: ["Subtract the overlap from the tea total.", "12 − 5.", "7."] },
      // @q02
      { id: "U10L3-mcq-2", type: "mcq", category: "conceptual", prompt: "What is the FIRST region to fill in a Venn word problem?", options: [ { id: "a", text: "The overlap (both)" }, { id: "b", text: "The outside" }, { id: "c", text: "The tea-only slice" }, { id: "d", text: "The whole universe" } ], correctOptionId: "a", diagnoses: { b: "The outside comes LAST — it's what's left.", c: "You need the overlap first to subtract it from the total.", d: "The universe is given, not filled." }, explanation: "Write the overlap first — it's subtracted from both totals.", hints: ["Which count is inside both circles?", "The overlap.", "The 'both' region."] },
      // @q03
      { id: "U10L3-mcq-3", type: "mcq", category: "word", prompt: "30 people: 18 read books, 14 watch films, 6 do both. How many do NEITHER?", options: [ { id: "a", text: "4" }, { id: "b", text: "26" }, { id: "c", text: "10" }, { id: "d", text: "6" } ], correctOptionId: "a", diagnoses: { b: "26 is 32 − 6 — you forgot to subtract from the 30 universe before answering.", c: "Book-only = 12 and film-only = 8 are 20, but the answer needs the outside too.", d: "6 is the overlap, not the outside." }, explanation: "book-only 12 + film-only 8 + both 6 = 26; 30 − 26 = 4 neither.", hints: ["Find the three inside regions first.", "30 − (12 + 8 + 6).", "4."] },
      // @q04
      { id: "U10L3-mcq-4", type: "mcq", category: "procedural", prompt: "40 people: 15 jog, 20 swim, 8 do both. How many jog ONLY?", options: [ { id: "a", text: "7" }, { id: "b", text: "15" }, { id: "c", text: "8" }, { id: "d", text: "12" } ], correctOptionId: "a", diagnoses: { b: "15 includes the 8 who also swim.", c: "8 is the overlap.", d: "12 is 20 − 8 — the swim-only count." }, explanation: "Jog-only = 15 − 8 = 7.", hints: ["Subtract the overlap from the jog total.", "15 − 8.", "7."] },
      // @q05
      ...
      // @q06
      { id: "U10L3-mcq-6", type: "mcq", category: "word", prompt: "A class of 25: 10 play chess, 14 play the piano, 6 do both. How many play NO instrument or game? (Those outside both circles.)", options: [ { id: "a", text: "7" }, { id: "b", text: "18" }, { id: "c", text: "12" }, { id: "d", text: "6" } ], correctOptionId: "a", diagnoses: { b: "18 = 10 + 14 − 6 — that's the union, then you still need 25 − 18.", c: "12 is chess-only — the outside is different.", d: "6 is the overlap, not the outside." }, explanation: "chess-only 4 + piano-only 8 + both 6 = 18; 25 − 18 = 7 neither.", hints: ["Fill the three inside regions first.", "25 − 18.", "7."] },
      // @q07
      { id: "U10L3-num-1", type: "numeric-input", category: "procedural", prompt: "24 students: 10 play tennis, 12 play football, 4 play both. How many play tennis ONLY? Type the number.", answer: 6, tolerance: 0, explanation: "Tennis-only = 10 − 4 = 6.", hints: ["Subtract the overlap from tennis.", "10 − 4.", "6."] },
      // @q08
      { id: "U10L3-num-2", type: "numeric-input", category: "procedural", prompt: "24 students: 10 play tennis, 12 play football, 4 play both. How many play NEITHER? Type the number.", answer: 6, tolerance: 0, explanation: "Union = 6 + 8 + 4 = 18; neither = 24 − 18 = 6.", hints: ["Find the three inside slices first.", "24 − 18.", "6."] },
      // @q09
      { id: "U10L3-num-3", type: "numeric-input", category: "conceptual", prompt: "50 people surveyed: 20 like apples, 25 like bananas, 10 like both. How many like bananas ONLY? Type the number.", answer: 15, tolerance: 0, explanation: "Bananas-only = 25 − 10 = 15.", hints: ["Subtract the overlap from the banana total.", "25 − 10.", "15."] },
      // @q10
      { id: "U10L3-frac-1", type: "fraction-input", category: "conceptual", prompt: "A class of 20: 12 like tea, 8 like coffee, 5 like both. What fraction of the class likes NEITHER? (Write it.)", numerator: 1, denominator: 4, acceptEquivalent: true, explanation: "Neither = 20 − 15 = 5, and 5/20 = 1/4.", hints: ["Neither = 5.", "5 out of 20.", "1/4."] },
      // @q11
      { id: "U10L3-tf-1", type: "true-false-justify", category: "conceptual", prompt: "20 students, 12 like tea, 8 like coffee, 5 like both: exactly 5 like neither.", isTrue: true, explanation: "Union = 12 + 8 − 5 = 15; neither = 20 − 15 = 5.", hints: ["Compute the union first.", "20 − 15.", "True — 5."] },
      // @q12
      { id: "U10L3-tf-2", type: "true-false-justify", category: "conceptual", prompt: "In the tea/coffee survey, the number who like tea ONLY is 12.", isTrue: false, explanation: "Tea-only subtracts the overlap: 12 − 5 = 7, not 12.", hints: ["The 12 includes the 'both' students.", "12 − 5.", "False — 7."] },
      // @q13
      ...
      // @q15
      { id: "U10L3-graph-1", type: "graph-interact", category: "word", prompt: "Set the slider to the number who like NEITHER: 20 students, 12 tea, 8 coffee, 5 both (key: value).", challenge: "Set the slider to 5.", validate: { value: 5 }, tolerance: 0, explanation: "Union 15, outside = 20 − 15 = 5.", hints: ["Union = 15.", "20 − 15.", "5."] }
    ],
