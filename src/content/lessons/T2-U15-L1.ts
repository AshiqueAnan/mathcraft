...
export const T2U15L1: Lesson = {
  ...
  quiz: {
    pool: [
      ...
      // @q02
      { id: "U15L1-mcq-2", type: "mcq", category: "conceptual", prompt: "Why is one equation with two unknowns NOT enough to pin down one answer?", options: [ { id: "a", text: "Equations are always wrong" }, { id: "b", text: "It has infinitely many solution points — a whole line of them" }, { id: "c", text: "There isn't a graph to draw" }, { id: "d", text: "The numbers are too big" } ], correctOptionId: "b", diagnoses: { a: "The equation is fine — it's just under-constrained.", c: "One equation IS a graph: a line of solutions.", d: "Size isn't the issue." }, explanation: "x + y = 10 is true for (0,10), (4,6), (9,1)… every point on the line. You need a second clue to cut it down.", hints: ["Pick three pairs that fit.", "All lie on one line.", "Need a second line."] },
      // @q03
      { id: "U15L1-mcq-3", type: "mcq", category: "word", prompt: "2 pens + 1 notebook = $7 and 1 pen + 2 notebooks = $8. If a pen is p and a notebook is n, which system is correct?", options: [ { id: "a", text: "2p + n = 8; p + 2n = 7" }, { id: "b", text: "p + n = 7; p + n = 8" }, { id: "c", text: "2p + n = 7; p + 2n = 8" }, { id: "d", text: "2p + 2n = 7; p + n = 8" } ], correctOptionId: "c", diagnoses: { b: "Counts differ: 2 pens, then 2 notebooks.", a: "First clue is $7, second $8 — not swapped.", d: "Two pens + one notebook, not two of each." }, explanation: "2p + n = 7 and p + 2n = 8 — each sentence becomes one equation.", hints: ["2 pens → 2p.", "1 notebook → n.", "2p + n = 7 and p + 2n = 8 — each sentence becomes one equation."] },
      // @q04
      { id: "U15L1-mcq-4", type: "mcq", category: "procedural", prompt: "a + b = 12 and a − b = 4. Which point fits BOTH?", options: [ { id: "a", text: "(12, 0)" }, { id: "b", text: "(4, 8)" }, { id: "c", text: "(6, 6)" }, { id: "d", text: "(8, 4)" } ], correctOptionId: "d", diagnoses: { b: "(4, 8): 4 − 8 = −4, not 4.", c: "(6, 6): 6 − 6 = 0, not 4.", a: "(12, 0): 12 + 0 = 12 but 12 − 0 = 12." }, explanation: "(8, 4): 8 + 4 = 12 ✓ and 8 − 4 = 4 ✓.", hints: ["Check the difference too.", "(8, 4): both check.", "(8, 4): 8 + 4 = 12 ✓ and 8 − 4 = 4 ✓."] },
      // @q05
      { id: "U15L1-mcq-5", type: "mcq", category: "conceptual", prompt: "Two lines are parallel (never cross). What does that mean for the system?", options: [ { id: "a", text: "No pair satisfies both — no solution" }, { id: "b", text: "Every point is a solution" }, { id: "c", text: "One equation is wrong" }, { id: "d", text: "There are exactly two solutions" } ], correctOptionId: "a", diagnoses: { b: "Same line means infinite; parallel means none.", c: "Both equations can be perfectly fine and still never meet.", d: "Two lines cross at most once." }, explanation: "No point lies on both parallel lines, so the system has no solution.", hints: ["Parallel = no crossing.", "No crossing = no shared point.", "No point lies on both parallel lines, so the system has no solution."] },
      // @q06
      { id: "U15L1-mcq-6", type: "mcq", category: "word", prompt: "3 apples + 2 bananas cost $11; 1 apple + 2 bananas cost $5. If a = apples and b = bananas, what are the prices?", options: [ { id: "a", text: "Apple $1, banana $3" }, { id: "b", text: "Apple $3, banana $1" }, { id: "c", text: "Apple $2, banana $2" }, { id: "d", text: "Apple $4, banana $1" } ], correctOptionId: "b", diagnoses: { a: "Check: 3(1) + 2(3) = 9, not 11.", c: "3(2) + 2(2) = 10, not 11.", d: "3(4) + 2(1) = 14, not 11." }, explanation: "3(3) + 2(1) = 11 ✓ and 1(3) + 2(1) = 5 ✓ — the crossing point.", hints: ["Test each pair in both clues.", "(3, 1): 9 + 2 = 11 and 3 + 2 = 5.", "3(3) + 2(1) = 11 ✓ and 1(3) + 2(1) = 5 ✓ — the crossing point."] },
      // @q07
      { id: "U15L1-num-1", type: "numeric-input", category: "procedural", prompt: "x + y = 10 and y = x + 2. Type the x-coordinate of the crossing.", answer: 4, tolerance: 0, explanation: "x + (x + 2) = 10 → 2x = 8 → x = 4.", hints: ["Substitute y = x + 2.", "2x + 2 = 10.", "x + (x + 2) = 10 → 2x = 8 → x = 4."] },
      // @q08
      { id: "U15L1-num-2", type: "numeric-input", category: "procedural", prompt: "x + y = 10 and y = x + 2. Type the y-coordinate of the crossing.", answer: 6, tolerance: 0, explanation: "y = x + 2 = 4 + 2 = 6.", hints: ["Use x = 4.", "y = x + 2.", "y = x + 2 = 4 + 2 = 6."] },
      // @q09
      { id: "U15L1-num-3", type: "numeric-input", category: "conceptual", prompt: "2p + n = 7 and p + 2n = 8. Type the price of one pen (p).", answer: 2, tolerance: 0, explanation: "The lines cross at (2, 3): pen $2.", hints: ["Try p in both equations.", "p = 2 gives n = 3 in both.", "The lines cross at (2, 3): pen $2."] },
      // @q10
      { id: "U15L1-frac-1", type: "fraction-input", category: "conceptual", prompt: "x + y = 5 and y = x + 1. Write the x-coordinate as a fraction.", numerator: 2, denominator: 1, acceptEquivalent: true, explanation: "x + (x + 1) = 5 → 2x = 4 → x = 2.", hints: ["Substitute y.", "2x + 1 = 5.", "x + (x + 1) = 5 → 2x = 4 → x = 2."] },
      // @q11
      { id: "U15L1-tf-1", type: "true-false-justify", category: "conceptual", prompt: "(3, 4) is the solution of x + y = 7 and y = x + 1.", isTrue: true, explanation: "3 + 4 = 7 ✓ and 4 = 3 + 1 ✓ — both equations.", hints: ["Check sum.", "Check y = x + 1.", "3 + 4 = 7 ✓ and 4 = 3 + 1 ✓ — both equations."] },
      // @q12
      { id: "U15L1-tf-2", type: "true-false-justify", category: "conceptual", prompt: "One equation with two unknowns always has exactly one solution.", isTrue: false, explanation: "It has a whole line of solutions — you need two equations (two lines) to pin down a point.", hints: ["One line has many points.", "Crossing needs two lines.", "It has a whole line of solutions — you need two equations (two lines) to pin down a point."] },
      // @q13
