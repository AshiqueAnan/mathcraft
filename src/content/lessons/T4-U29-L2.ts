...
export const T4U29L2: Lesson = {
  ...
  quiz: {
    pool: [
      ...
      // @q02
      { id: "U29L2-mcq-2", type: "mcq", category: "conceptual", prompt: "The law of large numbers says…", options: [ { id: "a", text: "more trials → relative frequency approaches the true probability" }, { id: "b", text: "fewer trials → better estimates" }, { id: "c", text: "streaks must balance" }, { id: "d", text: "probability changes over time" } ], correctOptionId: "a", diagnoses: { b: "Fewer trials wobble MORE.", c: "That's the gambler's fallacy.", d: "The true probability is fixed." }, explanation: "The wobble shrinks as the sample grows.", hints: ["Large numbers.", "Converges.", "More trials."] },
      // @q03
      { id: "U29L2-mcq-3", type: "mcq", category: "word", prompt: "5 tails in a row. 'Heads is due next' is…", options: [ { id: "a", text: "the gambler's fallacy — each flip is independent" }, { id: "b", text: "correct — streaks must balance" }, { id: "c", text: "proven by the law of large numbers" }, { id: "d", text: "true after 10 tails" } ], correctOptionId: "a", diagnoses: { b: "No balancing mechanic exists.", c: "The law concerns many flips, not the next one.", d: "Independence never expires." }, explanation: "P(heads) = 0.5 on EVERY flip regardless of history.", hints: ["Independent flips.", "No memory.", "Gambler's fallacy."] },
      // @q04
      { id: "U29L2-mcq-4", type: "mcq", category: "procedural", prompt: "2000 flips: heads proportion is MOST likely…", options: [ { id: "a", text: "between 0.48 and 0.52" }, { id: "b", text: "exactly 0.5" }, { id: "c", text: "0.2" }, { id: "d", text: "0.8" } ], correctOptionId: "a", diagnoses: { b: "Exactly 0.5 is possible but not typical at 2000.", c: "0.2 is far from the fair coin.", d: "0.8 would need a biased coin." }, explanation: "Large samples land close to 0.5, rarely on it exactly.", hints: ["Near 0.5.", "0.48–0.52.", "Close range."] },
      // @q05
      { id: "U29L2-mcq-5", type: "mcq", category: "conceptual", prompt: "Relative frequency from 10 flips should be treated as…", options: [ { id: "a", text: "a rough hint, not the true probability" }, { id: "b", text: "the true probability exactly" }, { id: "c", text: "always 0.5" }, { id: "d", text: "meaningless" } ], correctOptionId: "a", diagnoses: { b: "Small samples wobble.", c: "A short run can give 0.7.", d: "It's a noisy estimate, still informative." }, explanation: "Tiny samples miss the long-run value.", hints: ["Small sample.", "Wobbles.", "Rough hint."] },
      // @q06
      { id: "U29L2-mcq-6", type: "mcq", category: "word", prompt: "A company says a product fails 1% of the time from 60 tests (no failures observed). The real failure rate is…", options: [ { id: "a", text: "unknown — 60 tests is too small to claim 0%" }, { id: "b", text: "definitely 0%" }, { id: "c", text: "definitely 1%" }, { id: "d", text: "exactly 60%" } ], correctOptionId: "a", diagnoses: { b: "No failures ≠ no failure rate.", c: "The claim is a theory, untested adequately.", d: "60% misreads counting." }, explanation: "Small samples can't confirm a rare 1% rate.", hints: ["Rare event.", "Needs many tests.", "Unknown."] },
      // @q07
      { id: "U29L2-num-1", type: "numeric-input", category: "procedural", prompt: "8 heads in 10 flips → relative frequency (decimal).", answer: 0.8, tolerance: 0.01, acceptFractions: true, explanation: "8 ÷ 10 = 0.8 — a short-run wobble above 0.5.", hints: ["8 ÷ 10.", "0.8.", "0.8."] },
      // @q08
      { id: "U29L2-num-2", type: "numeric-input", category: "procedural", prompt: "In 1000 flips, heads lands 515 times. Relative frequency = …", answer: 0.515, tolerance: 0.01, acceptFractions: true, explanation: "515 ÷ 1000 = 0.515 — close to 0.5.", hints: ["515 ÷ 1000.", "0.515.", "0.515."] },
      // @q09
      { id: "U29L2-num-3", type: "numeric-input", category: "conceptual", prompt: "How many total flips are in the two runs used above (10 + 1000)?", answer: 1010, tolerance: 0, explanation: "10 + 1000 = 1010 flips across both experiments.", hints: ["Add the runs.", "1010.", "1010."] },
      // @q10
      { id: "U29L2-frac-1", type: "fraction-input", category: "conceptual", prompt: "A die rolled 60 times shows the number 4 exactly 10 times. Relative frequency of rolling 4 is…", numerator: 1, denominator: 6, acceptEquivalent: true, explanation: "10/60 = 1/6 — matches theory here.", hints: ["10 ÷ 60.", "1/6.", "1/6."] },
      // @q11
      { id: "U29L2-tf-1", type: "true-false-justify", category: "conceptual", prompt: "The law of large numbers guarantees relative frequency equals the probability exactly.", isTrue: false, explanation: "It approaches it — equality in the limit, rarely exact at any finite sample.", hints: ["Approaches.", "Not exact.", "False."] },
      // @q12
      { id: "U29L2-tf-2", type: "true-false-justify", category: "conceptual", prompt: "A roulette wheel has no memory — past reds don't make black due.", isTrue: true, explanation: "Each spin is independent; the gambler's fallacy is false.", hints: ["Independent spins.", "No memory.", "True."] },
      // @q13
      { id: "U29L2-order-1", type: "order-steps", category: "word", prompt: "Order the steps to find relative frequency.", sequence: ["Count the successes", "Count total trials", "Divide successes ÷ total"], diagnoses: { "Count the successes@1": "Count successes first.", "Count total trials@0": "Then total trials.", "Divide successes ÷ total@0": "Divide last." }, explanation: "Successes, total, divide.", hints: ["Successes.", "Total.", "Divide."] },
      // @q14
      { id: "U29L2-drag-1", type: "drag-match", category: "conceptual", prompt: "Match each idea to its meaning.", pairs: [ { source: "Relative frequency", target: "observed ÷ trials" }, { source: "Theoretical probability", target: "favourable ÷ total (long run)" }, { source: "Gambler's fallacy", target: "streaks influence next trial" } ], diagnoses: { "Relative frequency->favourable ÷ total (long run)": "Observed runs give relative frequency.", "Theoretical probability->observed ÷ trials": "Theory is the long-run prediction.", "Gambler's fallacy->observed ÷ trials": "The fallacy misuses streaks." }, explanation: "Observed vs predicted — and the false belief that links them wrongly.", hints: ["Observed ÷ trials.", "Theory long-run.", "No memory."] },
      // @q15
      { id: "U29L2-graph-1", type: "graph-interact", category: "word", prompt: "Set the slider to the relative frequency of 40 heads in 80 flips (key: value).", challenge: "Set the slider to 0.5.", validate: { value: 0.5 }, tolerance: 0.01, explanation: "40 ÷ 80 = 0.5 — exactly fair here.", hints: ["40 ÷ 80.", "0.5.", "0.5."] }
    ],
