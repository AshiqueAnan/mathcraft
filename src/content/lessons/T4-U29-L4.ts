...
export const T4U29L4: Lesson = {
  ...
  quiz: {
    pool: [
      // @q01
      { id: "U29L4-mcq-1", type: "mcq", category: "procedural", prompt: "Flip a coin twice (independent). P(H then H) = …", options: [ { id: "a", text: "1/4" }, { id: "b", text: "1/2" }, { id: "c", text: "1" }, { id: "d", text: "0" } ], correctOptionId: "a", diagnoses: { b: "1/2 is one flip.", c: "1 is certainty.", d: "0 is impossible." }, explanation: "Independent → multiply: ½ × ½ = ¼.", hints: ["Multiply.", "½ × ½.", "Independent → multiply: ½ × ½ = ¼."] },
      // @q02
      { id: "U29L4-mcq-2", type: "mcq", category: "conceptual", prompt: "Independent events satisfy…", options: [ { id: "a", text: "P(A and B) = 0 always" }, { id: "b", text: "P(A and B) = P(A) × P(B)" }, { id: "c", text: "P(A and B) = P(A) + P(B)" }, { id: "d", text: "P(B) changes after A" } ], correctOptionId: "b", diagnoses: { a: "That's mutually exclusive.", c: "Addition is for OR with overlap.", d: "Independence means no change." }, explanation: "One outcome doesn't influence the other — the product rule applies.", hints: ["No influence.", "Multiply.", "One outcome doesn't influence the other — the product rule applies."] },
      // @q03
      { id: "U29L4-mcq-3", type: "mcq", category: "word", prompt: "Bag: 2 red, 3 blue. WITHOUT replacement, P(red then red) = …", options: [ { id: "a", text: "2/5 + 1/4" }, { id: "b", text: "2/5 × 2/5 = 4/25" }, { id: "c", text: "2/5 × 1/4 = 1/10" }, { id: "d", text: "1/2" } ], correctOptionId: "c", diagnoses: { b: "4/25 assumes replacement — the bag changed.", a: "Would need OR logic, not AND.", d: "1/2 isn't this product." }, explanation: "First red 2/5; one red removed leaves 1 red of 4 → multiply 2/5 × 1/4.", hints: ["Bag shrinks.", "2/5 then 1/4.", "First red 2/5; one red removed leaves 1 red of 4 → multiply 2/5 × 1/4."] },
      // @q04
      { id: "U29L4-mcq-4", type: "mcq", category: "procedural", prompt: "P(A) = 0.4, P(B) = 0.5, independent. P(A and B) = …", options: [ { id: "a", text: "0.45" }, { id: "b", text: "0.9" }, { id: "c", text: "0.1" }, { id: "d", text: "0.2" } ], correctOptionId: "d", diagnoses: { b: "0.9 is P(A or B) without subtracting overlap.", c: "0.1 subtracts — wrong operation.", a: "0.45 averages." }, explanation: "0.4 × 0.5 = 0.2.", hints: ["Multiply.", "0.4 × 0.5.", "0.4 × 0.5 = 0.2."] },
      // @q05
      ...
      // @q06
      { id: "U29L4-mcq-6", type: "mcq", category: "word", prompt: "P(A) = 0.5, P(B) = 0.3, P(A and B) = 0.15. P(A or B) = …", options: [ { id: "a", text: "0.8" }, { id: "b", text: "0.65" }, { id: "c", text: "0.5" }, { id: "d", text: "0.35" } ], correctOptionId: "b", diagnoses: { a: "0.8 adds without subtracting the overlap.", c: "0.5 is just P(A).", d: "0.35 subtracts everything." }, explanation: "0.5 + 0.3 − 0.15 = 0.65 — subtract the double-counted overlap.", hints: ["Add, subtract overlap.", "0.8 − 0.15.", "0.5 + 0.3 − 0.15 = 0.65 — subtract the double-counted overlap."] },
      // @q07
      { id: "U29L4-num-1", type: "numeric-input", category: "procedural", prompt: "P(A) = 0.6, P(B) = 0.5, independent. P(A and B) (decimal).", answer: 0.3, tolerance: 0.01, acceptFractions: true, explanation: "0.6 × 0.5 = 0.3.", hints: ["Multiply.", "0.3.", "0.6 × 0.5 = 0.3."] },
      // @q08
      { id: "U29L4-num-2", type: "numeric-input", category: "procedural", prompt: "Bag: 1 red, 2 blue. WITH replacement, P(red then red) as a fraction of 9ths (denominator).", answer: 9, tolerance: 0, explanation: "1/3 × 1/3 = 1/9 — denominator 9.", hints: ["1/3 × 1/3.", "1/9.", "1/3 × 1/3 = 1/9 — denominator 9."] },
      // @q09
      { id: "U29L4-num-3", type: "numeric-input", category: "conceptual", prompt: "P(A or B) when A and B are mutually exclusive with P(A)=0.4, P(B)=0.35.", answer: 0.75, tolerance: 0.01, acceptFractions: true, explanation: "No overlap → just add: 0.4 + 0.35 = 0.75.", hints: ["No overlap to subtract.", "0.4 + 0.35.", "No overlap → just add: 0."] },
      // @q10
      { id: "U29L4-frac-1", type: "fraction-input", category: "conceptual", prompt: "Bag: 3 red, 1 blue, WITHOUT replacement. P(blue then blue) = 1/4 × 0 = 0. What fraction of the first draw was blue?", numerator: 1, denominator: 4, acceptEquivalent: true, explanation: "1 blue of 4 marbles → 1/4.", hints: ["1 of 4.", "1/4.", "1 blue of 4 marbles → 1/4."] },
      // @q11
      { id: "U29L4-tf-1", type: "true-false-justify", category: "conceptual", prompt: "Independent events can still both happen.", isTrue: true, explanation: "Independence ≠ exclusivity — coin flips both can land heads.", hints: ["Both possible.", "Product rule.", "Independence ≠ exclusivity — coin flips both can land heads."] },
      // @q12
      { id: "U29L4-tf-2", type: "true-false-justify", category: "conceptual", prompt: "Without replacement, the second draw is independent of the first.", isTrue: false, explanation: "The bag changes → the probabilities shift → dependence.", hints: ["Bag shifts.", "Dependent.", "The bag changes → the probabilities shift → dependence."] },
      // @q13
      { id: "U29L4-order-1", type: "order-steps", category: "word", prompt: "Order the steps for P(red then red) without replacement (2 red, 3 blue).", sequence: ["First draw: 2/5 red", "Second: 1 red left of 4 → 1/4", "Multiply: 2/5 × 1/4 = 1/10"], diagnoses: { "First draw: 2/5 red@1": "First draw first.", "Second: 1 red left of 4 → 1/4@0": "Then the changed bag.", "Multiply: 2/5 × 1/4 = 1/10@0": "Multiply last." }, explanation: "First, changed second, multiply.", hints: ["2/5.", "1/4.", "First, changed second, multiply."] },
      // @q14
      { id: "U29L4-drag-1", type: "drag-match", category: "conceptual", prompt: "Match each situation to its probability link.", pairs: [ { source: "With replacement", target: "independent" }, { source: "Without replacement", target: "dependent" }, { source: "Coin flips", target: "independent" } ], diagnoses: { "With replacement->dependent": "Replacement resets the state — independent.", "Without replacement->independent": "The bag changes — dependent.", "Coin flips->dependent": "Coins don't remember — independent." }, explanation: "Whether the state resets decides independence.", hints: ["Replace = reset.", "No replace = shift.", "Whether the state resets decides independence."] },
      // @q15
      { id: "U29L4-graph-1", type: "graph-interact", category: "word", prompt: "Bag: 2 red, 2 blue. WITHOUT replacement, set the slider to P(red then red) (key: value).", challenge: "Bag: 2 red, 2 blue. WITHOUT replacement, — adjust the values below to match the condition.", validate: { value: 0.17 }, tolerance: 0.02, explanation: "2/4 × 1/3 = 1/6 ≈ 0.167.", hints: ["2/4 × 1/3.", "1/6.", "2/4 × 1/3 = 1/6 ≈ 0."] }
    ],
