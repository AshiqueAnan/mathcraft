...
export const T3U27L2: Lesson = {
  ...
  intuitionBlocks: [{ widget: "geometry-playground", props: { mode: "enlargement", scale: 2 }, narrative: "Place a centre and drag the scale factor slider: each vertex moves k times farther from the centre along the same ray. k = 2 makes side lengths 2× and area 4×; k = 0.5 shrinks both. Negative k flips to the opposite side — a bigger, flipped copy." }],
  ...
  quiz: {
    pool: [
      ...
      // @q03
      { id: "U27L2-mcq-3", type: "mcq", category: "word", prompt: "A photo enlarged by k = 2. Its area becomes…", options: [ { id: "a", text: "8× the original" }, { id: "b", text: "2× the original" }, { id: "c", text: "4× the original" }, { id: "d", text: "the same" } ], correctOptionId: "c", diagnoses: { b: "2× is the LINEAR scale, area squares it.", a: "8× would need 3-D scaling.", d: "Scaling changes area unless k = 1." }, explanation: "Area scales by k² = 2² = 4.", hints: ["k² = 4.", "Area × 4.", "Area scales by k² = 2² = 4."] },
      // @q04
      { id: "U27L2-mcq-4", type: "mcq", category: "procedural", prompt: "Enlarge (3, −2) by k = −1 about the origin. New point = …", options: [ { id: "a", text: "(0, 0)" }, { id: "b", text: "(3, 2)" }, { id: "c", text: "(−3, −2)" }, { id: "d", text: "(−3, 2)" } ], correctOptionId: "d", diagnoses: { b: "(3, 2) reflects across the x-axis only.", c: "(−3, −2) is the k = 1 point unchanged.", a: "(0, 0) would be the centre itself." }, explanation: "k = −1 sends each point to the origin-side opposite: (−3, 2).", hints: ["Negate both.", "(−3, 2).", "k = −1 sends each point to the origin-side opposite: (−3, 2)."] },
      // @q05
      { id: "U27L2-mcq-5", type: "mcq", category: "conceptual", prompt: "Under enlargement, angles…", options: [ { id: "a", text: "stay the same" }, { id: "b", text: "multiply by k" }, { id: "c", text: "halve" }, { id: "d", text: "flip sign" } ], correctOptionId: "a", diagnoses: { b: "Angles don't scale — the shape just resizes.", c: "Halving isn't part of enlargement.", d: "Only k < 0 flips orientation, not angle sizes." }, explanation: "Enlargement is a similar-copy transformation — angles are preserved.", hints: ["Similar shapes.", "Angles unchanged.", "Enlargement is a similar-copy transformation — angles are preserved."] },
      // @q06
      { id: "U27L2-mcq-6", type: "mcq", category: "word", prompt: "A poster printed at 150% (k = 1.5). Its area is about…", options: [ { id: "a", text: "1.5× the original" }, { id: "b", text: "2.25× the original" }, { id: "c", text: "3× the original" }, { id: "d", text: "the same" } ], correctOptionId: "b", diagnoses: { a: "1.5× is the linear scale; area squares it.", c: "3× doubles 1.5 — not the area rule.", d: "Any k ≠ 1 changes area." }, explanation: "Area scales by k² = 1.5² = 2.25.", hints: ["1.5².", "2.25×.", "Area scales by k² = 1."] },
      // @q07
      { id: "U27L2-num-1", type: "numeric-input", category: "procedural", prompt: "Enlarge (4, −3) by k = 2 about the origin. New x-coordinate = …", answer: 8, tolerance: 0, explanation: "4 × 2 = 8.", hints: ["× 2.", "8.", "4 × 2 = 8."] },
      // @q08
      { id: "U27L2-num-2", type: "numeric-input", category: "procedural", prompt: "A square of side 3 is enlarged by k = 3. New side length = …", answer: 9, tolerance: 0, explanation: "3 × 3 = 9.", hints: ["× 3.", "9.", "3 × 3 = 9."] },
      // @q09
      { id: "U27L2-num-3", type: "numeric-input", category: "conceptual", prompt: "A triangle's area is 5 cm², enlarged by k = 2. New area in cm².", answer: 20, tolerance: 0, unit: "cm²", explanation: "5 × 2² = 20 cm².", hints: ["× k² = 4.", "5 × 4.", "5 × 2² = 20 cm²."] },
      // @q10
      { id: "U27L2-frac-1", type: "fraction-input", category: "conceptual", prompt: "An enlargement by k = 0.5: the image's area is what fraction of the original?", numerator: 1, denominator: 4, acceptEquivalent: true, explanation: "k² = 0.5² = 0.25 = 1/4.", hints: ["0.5² = 0.25.", "1/4.", "k² = 0.5² = 0.25 = 1/4."] },
      // @q11
      { id: "U27L2-tf-1", type: "true-false-justify", category: "conceptual", prompt: "k = −2 enlarges AND flips the shape to the opposite side of the centre.", isTrue: true, explanation: "Negative k mirrors through the centre while scaling by 2.", hints: ["Sign = flip.", "2 = scale.", "Negative k mirrors through the centre while scaling by 2."] },
      // @q12
      { id: "U27L2-tf-2", type: "true-false-justify", category: "conceptual", prompt: "Area scales by k, not k².", isTrue: false, explanation: "Two dimensions each scale by k, so area scales by k × k = k².", hints: ["Length × width.", "k × k.", "Two dimensions each scale by k, so area scales by k × k = k²."] },
      // @q13
      { id: "U27L2-order-1", type: "order-steps", category: "word", prompt: "Order the steps to enlarge (3, 4) by k = 2 about the origin.", sequence: ["Measure from centre: (3, 4)", "Multiply each coordinate by 2", "Image: (6, 8)"], diagnoses: { "Measure from centre: (3, 4)@1": "Measure from the centre first.", "Multiply each coordinate by 2@0": "Then multiply.", "Image: (6, 8)@0": "State the image last." }, explanation: "Measure, multiply, report.", hints: ["(3, 4).", "× 2.", "Measure, multiply, report."] },
      // @q14
      { id: "U27L2-drag-1", type: "drag-match", category: "conceptual", prompt: "Match each change to its scaling rule.", pairs: [ { source: "Side length", target: "× k" }, { source: "Area", target: "× k²" }, { source: "Angle", target: "unchanged" } ], diagnoses: { "Side length->× k²": "Length scales linearly by k.", "Area->× k": "Area squares the factor.", "Angle->× k": "Angles are preserved." }, explanation: "Length × k, area × k², angles unchanged.", hints: ["Length k.", "Area k².", "Length × k, area × k², angles unchanged."] },
      // @q15
      { id: "U27L2-graph-1", type: "graph-interact", category: "word", prompt: "Enlarge (2, 3) by k = 4 about the origin. Set the slider to the NEW X-coordinate (key: value).", challenge: "Enlarge (2, 3) by k = 4 about the origin. — adjust the values below to match the condition.", validate: { value: 8 }, tolerance: 0.01, explanation: "2 × 4 = 8.", hints: ["× 4.", "8.", "2 × 4 = 8."] }
    ],
