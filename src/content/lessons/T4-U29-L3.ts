...
export const T4U29L3: Lesson = {
  ...
  quiz: {
    pool: [
      // @q01
      { id: "U29L3-mcq-1", type: "mcq", category: "procedural", prompt: "Two fair coins flipped. P(HH) = …", options: [ { id: "a", text: "1/4" }, { id: "b", text: "1/2" }, { id: "c", text: "1/8" }, { id: "d", text: "2/4" } ], correctOptionId: "a", diagnoses: { b: "1/2 is P(first flip heads).", c: "1/8 would need three coins.", d: "2/4 counts two paths, not HH." }, explanation: "Along the tree: ½ × ½ = ¼.", hints: ["Multiply branches.", "½ × ½.", "Along the tree: ½ × ½ = ¼."] },
      // @q02
      { id: "U29L3-mcq-2", type: "mcq", category: "conceptual", prompt: "Why are HT and TH counted as DIFFERENT outcomes?", options: [ { id: "a", text: "They're identical" }, { id: "b", text: "The tree orders coin 1 then coin 2" }, { id: "c", text: "Heads is always first" }, { id: "d", text: "Order never matters" } ], correctOptionId: "b", diagnoses: { a: "They're distinct paths in the sample space.", c: "Heads can be on either coin.", d: "Order matters here — that's the point." }, explanation: "Coin 1 H/coin 2 T is a separate path from coin 1 T/coin 2 H.", hints: ["First vs second coin.", "Two paths.", "Different outcomes."] },
      // @q03
      { id: "U29L3-mcq-3", type: "mcq", category: "word", prompt: "A spinner (2 equal halves: A, B) spun TWICE. P(AA) = …", options: [ { id: "a", text: "1/3" }, { id: "b", text: "1/2" }, { id: "c", text: "1/4" }, { id: "d", text: "2/4" } ], correctOptionId: "c", diagnoses: { b: "1/2 is one spin landing A.", a: "1/3 isn't from equally likely halves.", d: "2/4 would be P(A on either spin)." }, explanation: "Branch ½ then ½ → ¼ — the tree model applies to any two-stage trial.", hints: ["½ × ½.", "¼.", "Branch ½ then ½ → ¼ — the tree model applies to any two-stage trial."] },
      // @q04
      { id: "U29L3-mcq-4", type: "mcq", category: "procedural", prompt: "Roll a die, then flip a coin. Number of paths in the sample space = …", options: [ { id: "a", text: "2" }, { id: "b", text: "8" }, { id: "c", text: "6" }, { id: "d", text: "12" } ], correctOptionId: "d", diagnoses: { b: "8 is 2³ — wrong multiplier.", c: "6 ignores the coin.", a: "2 ignores the die." }, explanation: "6 die outcomes × 2 coin outcomes = 12 paths.", hints: ["6 × 2.", "12.", "6 die outcomes × 2 coin outcomes = 12 paths."] },
      // @q05
      { id: "U29L3-mcq-5", type: "mcq", category: "conceptual", prompt: "The sum of all leaf probabilities on a tree equals…", options: [ { id: "a", text: "1" }, { id: "b", text: "0.5" }, { id: "c", text: "2" }, { id: "d", text: "the number of leaves" } ], correctOptionId: "a", diagnoses: { b: "0.5 is one flipped coin's branch sum.", c: "2 has no probabilistic meaning.", d: "Leaves count, but their probs sum to 1." }, explanation: "Every possible outcome is covered — the full sample space totals 1.", hints: ["All outcomes.", "Certainty.", "Every possible outcome is covered — the full sample space totals 1."] },
      // @q06
      { id: "U29L3-mcq-6", type: "mcq", category: "word", prompt: "Two coins: P(exactly one head) = …", options: [ { id: "a", text: "1/4" }, { id: "b", text: "1/2" }, { id: "c", text: "3/4" }, { id: "d", text: "1" } ], correctOptionId: "b", diagnoses: { a: "1/4 is P(HH) alone.", c: "3/4 is P(at least one head).", d: "1 is certainty." }, explanation: "P(HT) + P(TH) = ¼ + ¼ = ½ — ADD the two distinct paths.", hints: ["HT + TH.", "¼ + ¼.", "P(HT) + P(TH) = ¼ + ¼ = ½ — ADD the two distinct paths."] },
      // @q07
      { id: "U29L3-num-1", type: "numeric-input", category: "procedural", prompt: "Two fair coins: P(TT) as a fraction (denominator).", answer: 4, tolerance: 0, explanation: "¼ — denominators: 4 paths.", hints: ["½ × ½.", "¼.", "¼ — denominators: 4 paths."] },
      // @q08
      { id: "U29L3-num-2", type: "numeric-input", category: "procedural", prompt: "Two fair coins: P(exactly one tail) as a decimal.", answer: 0.5, tolerance: 0.01, acceptFractions: true, explanation: "P(HT) + P(TH) = ¼ + ¼ = ½.", hints: ["Two paths.", "¼ + ¼ = ½.", "P(HT) + P(TH) = ¼ + ¼ = ½."] },
      // @q09
      { id: "U29L3-num-3", type: "numeric-input", category: "conceptual", prompt: "A die, then a coin: how many branches would a tree draw for the die?", answer: 6, tolerance: 0, explanation: "Six first-stage branches, one per die face.", hints: ["Die faces.", "6.", "Six first-stage branches, one per die face."] },
      // @q10
      { id: "U29L3-frac-1", type: "fraction-input", category: "conceptual", prompt: "Two coins: P(at least one head). All leaves except TT.", numerator: 3, denominator: 4, acceptEquivalent: true, explanation: "HH, HT, TH are 3 of 4 leaves — 3/4.", hints: ["3 of 4 leaves.", "3/4.", "HH, HT, TH are 3 of 4 leaves — 3/4."] },
      // @q11
      { id: "U29L3-tf-1", type: "true-false-justify", category: "conceptual", prompt: "A tree multiplies along branches and adds across alternative paths.", isTrue: true, explanation: "Multiplying joins stages; adding joins distinct favourable paths.", hints: ["× along, + across.", "True.", "Multiplying joins stages; adding joins distinct favourable paths."] },
      // @q12
      { id: "U29L3-tf-2", type: "true-false-justify", category: "conceptual", prompt: "P(HH) equals P(HT) for two fair coins — both are 1/4.", isTrue: true, explanation: "Each ordered path has the same ½ × ½ product.", hints: ["Ordered paths.", "Both ¼.", "Each ordered path has the same ½ × ½ product."] },
      // @q13
      { id: "U29L3-order-1", type: "order-steps", category: "word", prompt: "Order the steps to find P(HH) with two coins via a tree.", sequence: ["Draw coin-1 branches: H, T (each ½)", "Split each into coin-2 branches", "Multiplying: HH = ½ × ½ = ¼"], diagnoses: { "Draw coin-1 branches: H, T (each ½)@1": "Start with the first stage.", "Split each into coin-2 branches@0": "Then the second stage.", "Multiplying: HH = ½ × ½ = ¼@0": "Multiply last." }, explanation: "First stage, second stage, multiply the path.", hints: ["Coin 1.", "Coin 2.", "First stage, second stage, multiply the path."] },
      // @q14
      { id: "U29L3-drag-1", type: "drag-match", category: "conceptual", prompt: "Match each event to its two-coin probability.", pairs: [ { source: "HH", target: "1/4" }, { source: "Exactly one head", target: "1/2" }, { source: "At least one head", target: "3/4" } ], diagnoses: { "HH->1/2": "HH is one path: 1/4.", "Exactly one head->3/4": "HT + TH = 1/2.", "At least one head->1/4": "Three of four leaves: 3/4." }, explanation: "Each event gathers specific leaves.", hints: ["HH = ¼.", "One head = ½.", "Each event gathers specific leaves."] },
      // @q15
      { id: "U29L3-graph-1", type: "graph-interact", category: "word", prompt: "Two coins: set the slider to P(HT or TH) (key: value).", challenge: "Adjust the values below so they match the condition described in the prompt.", validate: { value: 0.5 }, tolerance: 0.01, explanation: "¼ + ¼ = 0.5.", hints: ["Two paths.", "¼ + ¼.", "¼ + ¼ = 0.5."] }
    ],
