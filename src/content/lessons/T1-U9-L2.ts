...
export const T1U9L2: Lesson = {
  ...
  quiz: {
    pool: [
      // @q01
      { id: "U9L2-mcq-1", type: "mcq", category: "procedural", prompt: "How many significant figures does 0.00304 have?", options: [ { id: "a", text: "Three" }, { id: "b", text: "Five" }, { id: "c", text: "Four" }, { id: "d", text: "Two" } ], correctOptionId: "a", diagnoses: { b: "Leading zeros don't count — they only set the scale.", c: "Check which zeros sit BETWEEN digits.", d: "The 0 between 3 and 4 IS significant." }, explanation: "3, 0 (between), and 4 are significant — leading zeros never count.", hints: ["Leading zeros are placeholders.", "Count from the first non-zero.", "3, 0 (between), and 4 are significant — leading zeros never count."] },
      // @q02
      { id: "U9L2-mcq-2", type: "mcq", category: "conceptual", prompt: "Which number claims the MOST precision (most sig figs)?", options: [ { id: "a", text: "5.60" }, { id: "b", text: "5.600" }, { id: "c", text: "5.6" }, { id: "d", text: "5" } ], correctOptionId: "b", diagnoses: { a: "5.60 has three sig figs — good, but not the most here.", c: "5.6 has two sig figs.", d: "5 has just one sig fig." }, explanation: "5.600 has FOUR sig figs — every trailing zero after the point counts.", hints: ["Count trailing zeros after the decimal point.", "5.600 keeps all of them.", "5.600 has FOUR sig figs — every trailing zero after the point counts."] },
      // @q03
      { id: "U9L2-mcq-3", type: "mcq", category: "word", prompt: "A sign says the lake is 12.0 km away. What does the '.0' honestly claim?", options: [ { id: "a", text: "The lake is a tenth of a km wide" }, { id: "b", text: "It is exactly 12 km" }, { id: "c", text: "Measured to the nearest tenth of a km" }, { id: "d", text: "Nothing — zeros are decorations" } ], correctOptionId: "c", diagnoses: { b: "It's rounded to a tenth — could really be 11.96 or 12.04.", a: "Location, not size.", d: "The zero after the point IS significant — it narrows the range." }, explanation: "12.0 is rounded to the nearest tenth, so the true distance sits between 11.95 and 12.05 km.", hints: ["What place does the digit after the point mark?", "Tenths.", "Nearest tenth of a km."] },
      // @q04
      { id: "U9L2-mcq-4", type: "mcq", category: "procedural", prompt: "Round 4,652 to TWO significant figures.", options: [ { id: "a", text: "5,000" }, { id: "b", text: "4,600" }, { id: "c", text: "4,650" }, { id: "d", text: "4,700" } ], correctOptionId: "d", diagnoses: { b: "You truncated instead of rounding — 5 makes 6 go up.", c: "That's 3–4 sig figs, and still truncating the 5.", a: "Too aggressive — rounding to one sig fig." }, explanation: "4,652 → keep 4,6; the next digit 5 rounds 6 up to 7 → 4,700.", hints: ["Two sig figs means keep 4 and 6.", "Check the third digit: 5 rounds up.", "4,652 → keep 4,6; the next digit 5 rounds 6 up to 7 → 4,700."] },
      // @q05
      ...
      // @q06
      { id: "U9L2-mcq-6", type: "mcq", category: "word", prompt: "A lab scale reads 25.40 g. The measurement is honest to how many significant figures?", options: [ { id: "a", text: "Three" }, { id: "b", text: "Four" }, { id: "c", text: "Five" }, { id: "d", text: "Two" } ], correctOptionId: "b", diagnoses: { a: "The final 0 was measured too — 25.40 is to hundredths.", c: "The 2,5,4,0 = 4; there is no fifth digit.", d: "Under-counting the measured zeros." }, explanation: "2, 5, 4, and the trailing 0 after the point are all significant → four sig figs.", hints: ["Trailing zeros after the point count.", "Count 2, 5, 4, 0.", "2, 5, 4, and the trailing 0 after the point are all significant → four sig figs."] },
      // @q07
      { id: "U9L2-num-1", type: "numeric-input", category: "procedural", prompt: "How many significant figures are in 12.30? Type the count.", answer: 4, tolerance: 0, explanation: "1, 2, 3, and the trailing 0 after the point → four sig figs.", hints: ["Trailing zeros after the decimal point count.", "1, 2, 3, 0.", "1, 2, 3, and the trailing 0 after the point → four sig figs."] },
      // @q08
      { id: "U9L2-num-2", type: "numeric-input", category: "procedural", prompt: "Round 8,420 to TWO significant figures. Type the rounded value.", answer: 8400, tolerance: 0, explanation: "Keep 8 and 4; third digit 2 → no change → 8,400.", hints: ["Keep the first two digits.", "2 is below 5 — no rounding up.", "Keep 8 and 4; third digit 2 → no change → 8,400."] },
      // @q09
      { id: "U9L2-num-3", type: "numeric-input", category: "conceptual", prompt: "Round 0.00749 to TWO significant figures. Type the power of ten in your answer (n in a×10ⁿ).", answer: -3, tolerance: 0, explanation: "0.00749 → 7.5 × 10⁻³ — keep 7 and 4; the next digit 9 rounds the 4 up to 5 → 7.5.", hints: ["First non-zero digit: 7.", "Keep 7.4, then the 9 rounds the 4 up.", "0.00749 → 7.5 × 10⁻³ — keep 7 and 4; the next digit 9 rounds the 4 up to 5 → 7.5."] },
      // @q10
      { id: "U9L2-frac-1", type: "fraction-input", category: "conceptual", prompt: "0.1250 measured to four sig figs: write the value as a simplified fraction.", numerator: 1, denominator: 8, acceptEquivalent: true, explanation: "0.1250 = 125/1000 = 1/8 — the trailing zero means four sig figs.", hints: ["0.125 = 125/1000.", "Simplify.", "0.1250 = 125/1000 = 1/8 — the trailing zero means four sig figs."] },
      // @q11
      { id: "U9L2-tf-1", type: "true-false-justify", category: "conceptual", prompt: "0.0020 has two significant figures.", isTrue: true, explanation: "The leading zeros don't count; the 2 and the trailing 0 after the point do → two.", hints: ["Leading zeros are placeholders.", "2 and the final 0 count.", "The leading zeros don't count; the 2 and the trailing 0 after the point do → two."] },
      // @q12
      ...
      // @q15
      { id: "U9L2-graph-1", type: "graph-interact", category: "word", prompt: "This slider reads to the nearest hundred. Set it to the value of 4,700 (which has 2 sig figs) (key: value).", challenge: "This slider reads to the nearest hundred. — adjust the values below to match the condition.", validate: { value: 4700 }, tolerance: 0, explanation: "4,700 = two sig figs, marked to the nearest hundred.", hints: ["Two sig figs = nearest hundred.", "4700.", "4,700 = two sig figs, marked to the nearest hundred."] }
    ],
