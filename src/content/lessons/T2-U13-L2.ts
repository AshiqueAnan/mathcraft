...
export const T2U13L2: Lesson = {
  ...
  quiz: {
    pool: [
      // @q01
      { id: "U13L2-mcq-1", type: "mcq", category: "procedural", prompt: "Make x the subject of y = x + 3.", options: [ { id: "a", text: "x = y − 3" }, { id: "b", text: "x = y + 3" }, { id: "c", text: "x = 3 − y" }, { id: "d", text: "x = 3y" } ], correctOptionId: "a", diagnoses: { b: "Add 3 to isolate? That adds, not undoes.", c: "Wrong order — swap sides keeps signs.", d: "Multiplication isn't involved." }, explanation: "Undo +3: subtract 3 from both sides → x = y − 3.", hints: ["Undo the +3.", "Subtract 3 both sides.", "x = y − 3."] },
      // @q02
      { id: "U13L2-mcq-2", type: "mcq", category: "conceptual", prompt: "What does 'x is the subject' mean?", options: [ { id: "a", text: "x is alone on one side of the = " }, { id: "b", text: "x appears twice" }, { id: "c", text: "x is the biggest letter" }, { id: "d", text: "x is on the left always" } ], correctOptionId: "a", diagnoses: { b: "Subject means ALONE, not repeated.", c: "Size is irrelevant.", d: "Side doesn't matter — just alone." }, explanation: "The subject is the single letter isolated on one side: x = ….", hints: ["x = … with nothing else.", "Alone on one side.", "Alone on one side."] },
      // @q03
      { id: "U13L2-mcq-3", type: "mcq", category: "word", prompt: "F = ma is Newton's law. Make m the subject.", options: [ { id: "a", text: "m = F/a" }, { id: "b", text: "m = F × a" }, { id: "c", text: "m = a/F" }, { id: "d", text: "m = F − a" } ], correctOptionId: "a", diagnoses: { b: "Undo ×a by dividing, not multiplying.", c: "Divide by a, so m = F/a.", d: "Addition isn't involved." }, explanation: "m is multiplied by a: divide both sides by a → m = F/a.", hints: ["Undo the ×a.", "Divide by a.", "m = F/a."] },
      // @q04
      { id: "U13L2-mcq-4", type: "mcq", category: "procedural", prompt: "Make x the subject of y = 2x + 3.", options: [ { id: "a", text: "x = (y − 3)/2" }, { id: "b", text: "x = y/2 − 3" }, { id: "c", text: "x = (y + 3)/2" }, { id: "d", text: "x = 2y − 3" } ], correctOptionId: "a", diagnoses: { b: "Divide the whole side by 2 — not term by term.", c: "Subtract 3 first, not add.", d: "Wrong — 2y − 3 rearranges wrongly." }, explanation: "Undo +3 (y − 3 = 2x), then ÷2 → x = (y − 3)/2.", hints: ["Undo +3 first.", "Then ÷2.", "x = (y − 3)/2."] },
      // @q05
      { id: "U13L2-mcq-5", type: "mcq", category: "conceptual", prompt: "Why must you do the same to both sides when rearranging?", options: [ { id: "a", text: "To keep the equation true" }, { id: "b", text: "To make the numbers even" }, { id: "c", text: "To use bigger numbers" }, { id: "d", text: "It's just a rule" } ], correctOptionId: "a", diagnoses: { b: "Evenness is irrelevant.", c: "Value is never 'bigger'.", d: "It's not arbitrary — equality must hold." }, explanation: "An equation says both sides ARE equal; changing one side breaks the truth.", hints: ["What does '=' mean?", "Both sides equal.", "Keep it true."] },
      // @q06
      { id: "U13L2-mcq-6", type: "mcq", category: "word", prompt: "The perimeter P = 2(l + w). Make w the subject.", options: [ { id: "a", text: "w = P/2 − l" }, { id: "b", text: "w = P − l" }, { id: "c", text: "w = (P − 2)/l" }, { id: "d", text: "w = (P − l)/2" } ], correctOptionId: "a", diagnoses: { b: "Undo the ×2 first, then subtract l.", c: "l isn't divided — it's added inside.", d: "That undoes the wrong order." }, explanation: "P = 2(l + w): divide by 2 (P/2 = l + w), then −l → w = P/2 − l.", hints: ["Undo ×2 first.", "P/2 = l + w.", "Then subtract l."] },
      // @q07
      { id: "U13L2-num-1", type: "numeric-input", category: "procedural", prompt: "y = 3x − 2. Rearrange for x; type the number you add first.", answer: 2, tolerance: 0, explanation: "Add 2 to both sides first: y + 2 = 3x.", hints: ["Undo −2 first.", "Add 2.", "2."] },
      // @q08
      { id: "U13L2-num-2", type: "numeric-input", category: "procedural", prompt: "y = 3x after the +2 step: y + 2 = 3x. Type what you divide by to isolate x.", answer: 3, tolerance: 0, explanation: "x = (y + 2)/3 — divide by the coefficient 3.", hints: ["Undo the ×3.", "Divide by 3.", "3."] },
      // @q09
      { id: "U13L2-num-3", type: "numeric-input", category: "conceptual", prompt: "V = IR (voltage = current × resistance). Make R the subject; type the letter you divide by.", answer: 1, tolerance: 1, explanation: "R = V/I — divide by I (the current).", hints: ["Undo the ×I.", "Divide by I.", "R = V/I."] },
      // @q10
      { id: "U13L2-frac-1", type: "fraction-input", category: "conceptual", prompt: "y = 2x − 1. Rearrange for x. If y = 4, write x as a fraction.", numerator: 5, denominator: 2, acceptEquivalent: true, explanation: "x = (y + 1)/2 = (4 + 1)/2 = 5/2.", hints: ["Add 1: y + 1 = 2x.", "Divide by 2.", "x = 5/2."] },
      // @q11
      { id: "U13L2-tf-1", type: "true-false-justify", category: "conceptual", prompt: "Making u the subject of v = u + at gives u = v − at.", isTrue: true, explanation: "Subtract at from both sides isolates u.", hints: ["Undo the +at.", "Subtract at.", "True."] },
      // @q12
      { id: "U13L2-tf-2", type: "true-false-justify", category: "conceptual", prompt: "Making x the subject of y = 2x + 3 gives x = y/2 − 3.", isTrue: false, explanation: "Undo the +3 first: x = (y − 3)/2 — the −3 must be inside the bracket before dividing.", hints: ["Undo +3 before ×2.", "y − 3 = 2x.", "x = (y − 3)/2."] },
      // @q13
      { id: "U13L2-order-1", type: "order-steps", category: "word", prompt: "Order the steps to make x the subject of y = 2x + 3.", sequence: ["Subtract 3: y − 3 = 2x", "Divide by 2: (y − 3)/2 = x", "Write x = (y − 3)/2"], diagnoses: { "Divide by 2: (y − 3)/2 = x@0": "Subtract 3 first.", "Write x = (y − 3)/2@0": "Divide first." }, explanation: "Reverse the operations: undo +3, then ÷2.", hints: ["Undo the +3.", "Then ÷2.", "Write x = …."] },
      // @q14
      ...
      // @q15
      { id: "U13L2-graph-1", type: "graph-interact", category: "word", prompt: "Set the slider to u when v = 14, a = 3, t = 4 in v = u + at (key: value).", challenge: "Set the slider to 2.", validate: { value: 2 }, tolerance: 0, explanation: "u = v − at = 14 − 3×4 = 2.", hints: ["u = v − at.", "14 − 12.", "2."] }
    ],
