...
// Phase E gate: happy paths for T3-U25-L2 (AnimatedProof flagship) and
// T4-U29-L3 (TreeDiagramBuilder flagship). Answers derive from authored
// content via the shared helpers.
...
for (const lessonId of ["T3-U25-L2", "T4-U29-L3"]) {
  test(`fresh user masters ${lessonId}`, async ({ page }) => {
    ...
    // Widget-specific interaction: step the flagship proof / build the tree.
    if (lessonId === "T3-U25-L2") {
      const nextStep = surface.getByRole("button", { name: /Next|Step/ });
      if ((await nextStep.count()) > 0) await nextStep.first().click();
    } else {
      const branch = surface.locator("button");
      if ((await branch.count()) > 0) await branch.first().click();
    }
